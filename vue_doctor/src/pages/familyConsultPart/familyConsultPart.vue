<template>
  <div :class="['container',{'active':spread}]">
    <a  href="javascript:;" @click.stop="spread = !spread" class="chat ">图文咨询（半年）<span>{{set.switch  == '2' ? '已开通' : '未开通'}}</span><i class="icon-return-boldleft"></i></a>
    ...
    <router-link  to="/family/serviceIntro" class="chat chat_intro "> <i class="icon icon-intro"></i> 图文咨询（半年）服务介绍<i class="icon-return-boldleft"></i></router-link>
  </div>
</template>
<script>
  ...
  export default{
    ...
    data(){return{
      price :  ['998','2688','1288','自定义','1888'],
      priceIndex : 99,
      set : {num : '' ,switch : '',price : ''},
      initSwitch : '',
      spread : false,
      prompt : {show : false ,title : '请输入自定义价格',holder : '最多输入3988元',value : '',type : 'number',tip : '',limit : 2}
    }},
    created(){
      this.$api.gFamilySwitch().then(obj=>{
        delete obj.code;
        obj.switch =  obj.month['m6'] == 'on' ? '2' : '1';
        if(obj.price_six_months){
          obj.price = parseInt(obj.price_six_months);
          this.set =  obj;
          ...
          obj.price =obj.price_six_months;
        }
        this.set = obj;
    ...
    methods : {
      change(v){
        if(this.initSwitch == '2' && v == '1'){
          MessageBox.confirm('确定关闭图文咨询(半年)?').then(obj=>{
            this.set.switch = v;
          })
        }else{
          this.set.switch = v;
        ...
        if(i == 3){
          ...
          this.prompt.callback =(t)=>{
            if(t == '取消'){
              ...
              this.prompt.value = t;
              if(parseInt(t) > 3988){
                this.prompt.tip = '价格不得超过3988';
              }
              else if(parseInt(t) < 388){
                this.prompt.tip = '价格不得小于388';
              }
              else if(t){
                this.prompt.tip = '';
      ...
      submit(){
        ...
        if(this.set.switch == '2'){
          if(!set.price){
            Toast('请设置价格!');
            this.spread = true;
          }else if(parseInt(set.price) > 3988 || parseInt(set.price) < 388){
            Toast('价格范围388-3988!');
            this.spread = true;
          }else{
            this.$api.sFamilySwitch({
              price_six_months : set.price,
              m6 : set.switch == '2' ? 'on' : 'off'
            }).then(obj=>{
              this.$store.commit('IS_UPDATE_DOCINFO',true);
              this.$store.state.chatService[1] = true;
