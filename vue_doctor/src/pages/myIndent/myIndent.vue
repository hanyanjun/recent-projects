  ...
  <div class="container">
    ...
    <div class="content loadWrapper">
      <loading-more :data="arr" :pullupLoad="true" @pullup="load" :tip="isTip">
        <a href="javascript:;" v-for="item in arr"  v-if="item">
          ...
          <ul @click.stop="skip2(item)">
            ...
            <li>购买时间：<span>{{changeTime(4,item.create_time)}}</span></li>
            <!-- <li>购买时间：<span>{{$means.timestampTotime(item.created)}}</span></li> -->
            <li>价&nbsp;&nbsp;格：<span>{{item.pay_amount ? item.pay_amount : '免费'}}</span></li>
          </ul>
          <h6 v-show="item.order_status == '3'"><span>取消时间：{{changeTime(3,item.cancel_time)}}</span></h6>
...
export default{
    ...
    data(){return{
        arr : [''],
        isTip : false,
        filter : { order_type : '',ym : '' ,page : 1},
        control : true
//        empty :{show : false , text : '您还没有订单哦！'}
    }},
    components:{loadingMore,empty},
    created(){
      this.$store.commit('UPDATE_NAV',{title : '我的订单', fn : '筛选',fShow : false, hShow : true});
//更新待办事项
      this.$store.state.unReplay.status = true;
      let filter = this.$route.query.filter;
      if(filter){
          filter = JSON.parse(filter);
          this.filter.order_type = filter.type   == 'all' ? '' : filter.type;
          this.filter.ym = filter.ym == 'all' ? '' : filter.ym;
      }
      this.$api.gIndentList(this.filter).then(obj=>{
          this.arr  = obj.orders;
    ...
    mounted(){
        document.querySelector('.nav_submit').onclick =()=>{
             if(this.$route.query.filter){
                 this.$router.push({path : '/indentFilter', query : {filter : this.$route.query.filter}});
             }else{
               this.$router.push({path : '/indentFilter'});
             }
        }
    },
    methods:{
