...
function ajax(url,obj) {
    ...
    //2.打开与服务器的链接
	xhr = open(obj.method,url,false);
	for(let key  in obj.headers){
		var c = xhr.setRequestHeader
		debugger
...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
    ...
    construct: function(source, modelMetaData, editorId){
		...
		if(ORYX.CONFIG.BACKEND_SWITCH) {
			...
			var ht = new Ajax.Request(ACTIVITI.CONFIG.contextRoot + '/editor/stencilset?version=' + Date.now(), {
	            asynchronous: false,
				method: 'GET',
				headers : { 'auth': window.sessionStorage['temAuth']},
				onSuccess: this._init.bind(this),
				requestHeaders : { 'auth': window.sessionStorage['temAuth']},
				onFailure: this._cancelInit.bind(this),
			});
			// ResourceManager.__request('GET', 
			// 	ACTIVITI.CONFIG.contextRoot + '/editor/stencilset?version=' + Date.now(), 
			// 	null, this._init.bind(this) ,  
			// 	this._cancelInit.bind(this),
			// 	false,
			// 	{ 'auth': window.sessionStorage['temAuth']}
			// );
		} else {
			new Ajax.Request(source, {
	            asynchronous: false,
