...
.conClassWrap.companyCommon
  .head
    span.title 题库分类
    span.addClass.pointer(@click.stop='addClass') +新增分类
  .search
    search(v-model='searchT', @search='searchB',maxlength="20", holder='请输入分类名称')
  // tree编辑-选中-删除-新增
  .treeContent
    conTree(:treeData='treeData', @edit='edit', @selected='selected', @dele='dele', @add='add')
  reDialog(:visible.sync='diaShow', :title="type == 'add' ? '新增题库分类' : '编辑题库分类'", @close='close', @submit='submit')
    template(slot='body')
      el-form.form(:model='form', ref='form', :rules='rules', size='medium', label-width='100px')
        el-form-item.dialogForm(label='上级分类', size='medium', prop='parentId')
            ...
            el-option(v-for='item in treeData', :key='item.id', :label='item.className', :value='item.id')
              template(slot-scope='scope')
                h6(:style='`font-weight:normal;padding-left:${item._level*20}px`', @click='selectOptions(item,$event)')
                  span(style="font-size: 14px") {{item.className}}
                  i {{item.index}}
        el-form-item.dialogForm(label='分类名称', size='medium', prop='className')
          el-input(v-model.trim='form.className', placeholder='请输入分类名称')
        el-form-item.dialogForm(label='分类管理员', size='medium', prop='classManager')
          el-select(v-model='form.classManagerName', placeholder='请选择分类管理员')
            template(slot-scope='scope')
              selectMem(:treeData='treeMem.list', @select='selectMem', :clear='form.classManagerName')
...
export default {
  ...
  created(){
      ...
      // 1.获取tree列表
      // this.$api.getConClassTree().then(res => {
      ...
      searchB(){
          if(this.searchT.trim()){
            ...
            this.treeData.forEach((v)=>{
                ...
                if(newExp.test(v.className) && !isRex){
                    v._search = true;
                    v._isBtn = true;
                    if(v.parentId){
